...
// Set ADMIN_JWT_SECRET and ADMIN_PASSWORD as environment variables in production
const JWT_SECRET = process.env.ADMIN_JWT_SECRET || 'ylg-replace-this-secret-in-production';
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || 'YLGAdmin2025!';

const app = express();
app.use(cors({
    origin: process.env.ALLOWED_ORIGINS
        ? process.env.ALLOWED_ORIGINS.split(',')
        : ['http://localhost:3000', 'http://127.0.0.1:5500', 'https://www.ylgbullion.asia', 'https://ylgbullion.asia'],
    methods: ['GET', 'POST'],
...
const userSchema = new mongoose.Schema({
    ...
    // User Status: 'ACTIVE' (Normal), 'FROZEN' (Blocked)
...
app.post('/api/register', async (req, res) => {
    ...
    try {
        ...
        const newUser = new User({ userId, userName, password: hashed, balance: 1000 });
        await newUser.save();
...
app.post('/api/login', async (req, res) => {
    ...
    const user = await User.findOne({ userId });
    if (!user) {
        return res.status(400).json({ message: "ID သို့မဟုတ် Password မှားနေပါသည်" });
    }
    ...
    if (user.accountStatus === 'FROZEN') {
        return res.status(403).json({ message: "အကောင့် ပိတ်ထားပါသည်" });
    }
    res.status(200).json({
        userId: user.userId,
...
// ADMIN LOGIN — returns JWT for admin API access
...
app.post('/api/admin/control-user', adminAuth, async (req, res) => {
...
// 3.5. ADMIN FREEZE/UNFREEZE API
app.post('/api/admin/freeze-user', adminAuth, async (req, res) => {
    const { userId, freeze } = req.body; // freeze: true/false
    try {
        const status = freeze ? 'FROZEN' : 'ACTIVE';
        const user = await User.findOneAndUpdate({ userId }, { accountStatus: status }, { new: true });
        if (!user) return res.status(404).json({ message: "User ရှာမတွေ့ပါ" });
        res.json({ message: `User ${userId} ကို ${freeze ? 'အတားအဆီး' : 'ဖွင့်လည်း'}လိုက်ပါပြီ` });
    } catch (error) {
        res.status(500).json({ message: "Server Error" });
    }
});

// 3.6. ADMIN WITHDRAW API (Admin ပြင်ဆင်ချက်)
app.post('/api/admin/withdraw', adminAuth, async (req, res) => {
    try {
        ...
        const adminWithdraw = new Withdraw({
            userId,
        ...
        await adminWithdraw.save();

        res.json({ message: 'Manual withdrawal completed.', newBalance: user.balance });
    } catch (error) {
        res.status(500).json({ message: 'Failed to complete manual withdrawal.' });
    }
});

// 3.7. GET USER DATA API (Balance & Control status စစ်ရန်)
...
app.get('/api/admin/users', adminAuth, async (req, res) => {
    try {
        const users = await User.find({}, 'userId userName balance tradeControl accountStatus');
        res.json(users);
    } catch (error) {
...
app.post('/api/admin/adjust-pnl', adminAuth, async (req, res) => {
    try {
